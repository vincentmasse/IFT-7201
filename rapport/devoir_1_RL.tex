...
Nous avons aussi expérimenté pour connaître l'influence de la taille du noyau à l'intérieur du GaussianProcessRegressor et nous avons obtenu une meilleur performance avec une taille de 2 pour 5 instances différentes pour T = 1000 comme le montre la figure suivante. Il importe de nuancé le résultat ici car seulement 5 instances ont été utilisés (le temps de calcul de l'algorithme implanté étant passablement long). Avec plus de temps, plusieurs autres expériences auraient pu être présentées pour tenter de bien cerner l'effet de divers paramètres.
